...
export class CrudStockComponent implements OnInit,AfterViewInit {
 ...
 selectedCiterne=this.selectedCiternes;
  ...
  selectedTypepannes=[];
  selectedTypepanne=this.selectedTypepannes;
...
getSelectedDepots(selected) {
  ...
  getSelectedCiternes(selected) {
    this.selectedStock[0].idCiterne= selected;
   
  }
  getSelectedTypepanne(selected){
    this.selectedStock[0].id_TypePanne= selected;
  }
  loadCiterne() {
    var route = this.router
    this.loading = true
    this.dataService.loadParam("Citerne","Min").subscribe({
      next:
        res => {
          ...
          this.citerne = res // [{id:"",description:""},{...}]
    ...
    this.dataService.loadParam("TypePanne","Min").subscribe({
      next:
        res => {
          ...
          this.TypePanne = res // [{id:"",description:""},{...}]
  ...
  updateTypeMvt(something) {
    if(this.selectedStock[0].TypeMvt == 'S') this.selectedStock[0].TypeMvt = 'E'
    else this.selectedStock[0].TypeMvt = 'S'
  }
...
loadStores() {
  ...
  loadDataStore() {
      ...
      var route = this.router
      this.loading = true;
    this.dataService.loadStore("").subscribe({
        next: (d: any) => {
          this.loading = false;
          this.dataS= d;
          console.log("dataS",d)
...
if (!first) {
  ...
  this.dataService.loadMvt(urlParams).subscribe({
    next: (d: any) => {
...
loadDataBon() {
  ...
  var route = this.router
  this.loading = true;
this.dataService.loadMvt("?Bon=true").subscribe({
    next: (d: any) => {
      this.loading = false;
      this.dataB= d;
      console.log("dataB",d)
      this.dataB.forEach((e) => {
        e.DateMvt = this.tools.formatDateForInput(new Date(parseInt(e.DateMvt ?? "0") * 1000));
      })
  ...
  ajouter() {
   ...
   || !this.selectedStock[0].Reference
      ...
      for (let index = 1; index < this.selectedStock.length; index++) {
        ...
        this.selectedStock[index].TypeMvt=this.selectedStock[0].TypeMvt 
        this.selectedStock[index].idCiterne=this.selectedStock[0].idCiterne
        this.selectedStock[index].id_Vehicule=this.selectedStock[0].id_Vehicule 
        this.selectedStock[index].id_TypePanne=this.selectedStock[0].id_TypePanne
      }

      this.dataService.AddMvt(this.selectedStock)
        .pipe(
          catchError(err => {
            console.log("res", err)
  ...
  delete(stock) {
    if (confirm("Are you sure to delete " +stock)) {
      var route = this.router
      var p = "?Bon=" + stock;
      this.dataService.DeleteMvt(p).subscribe({
        next: (res) => {
          this.loadDataBon()
        }, error(err) {
          this.modalLoading = false;
  ...
  loadModify(ev) {
    ...
    this.selectedStock[0] =new Stock();
      ...
      if (ev) {
        ...
        var route = this.router
        this.dataService.loadMvt(url).subscribe({
          next: (res: any) => {
            if( res && res.length){
