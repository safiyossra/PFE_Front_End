...
export interface MyDateRangePickerDates {
    from: Date | {
        year: number,
        month: number,
        day: number
...
export interface MyDateRangePickerOptions {
...
export let DATERANGEPICKER_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => MyDateRangePickerComponent),
    multi: true
};

@Component({
    selector: 'my-daterangepicker',
    templateUrl: 'my-daterangepicker.component.html',
    styleUrls: ['my-daterangepicker.component.sass'],
    providers: [DATERANGEPICKER_VALUE_ACCESSOR]
})
export class MyDateRangePickerComponent implements ControlValueAccessor, OnInit, AfterViewInit, OnChanges {
    @ViewChild('fromInput', { static: true }) fromInput: ElementRef<HTMLDivElement>;
    @Input() options: MyDateRangePickerOptions;
    ...
    getDayOfWeek(day: number): string {
        const date = new Date();
        const dayOfWeek = dateFns.format(dateFns.setDay(date, day, {locale: locales[this.options.locale],weekStartsOn: 1}), 'eee');
    ...
    generateCalendar(): void {
        ...
        const days: IDay[] = dateFns.eachDayOfInterval({
            start: start,
            end: end,
          }).map(d => {
            ...
            return {
                ...
                isWithinRange: dateFns.isWithinInterval(d, {start:this.dateFrom, end: this.dateTo})
            };
        });
    ...
    selectDates(dates: MyDateRangePickerDates): void {
    ...
    selectRange(range: MyMenuItem): void {
        ...
        switch (unit) {
            ...
            case 'w':
                ...
                this.dateFrom = dateFns.startOfWeek(fromDate, {weekStartsOn: 1});
                this.dateTo = dateFns.endOfWeek(toDate, {weekStartsOn: 1});
                break;
            case 'y':
                if (amount) {
